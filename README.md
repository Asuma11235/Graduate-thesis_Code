...
主に CoIP（IP-MS）と AirID のデータを対象に，GO 解析・可視化・文献数集計・セット間比較を行う．さらに snAtlas 再解析（Seurat RDS を入力とする WGCNA / 発現量可視化）を収録した．
...
## 使用方法
- いずれも Rscript 想定。スクリプト冒頭のパス（例: `IN_XLSX`, `OUT_*`）を手元のファイルに合わせて書き換える．
- 必要パッケージはスクリプト内で自動インストール対応（Bioconductor: `clusterProfiler`, `org.At.tair.db` など）．
- 入力 Excel の列名想定:
  - GO 解析: `SYMBOL` と `Log2FC`（TAIR ID でも可）。
  - 交差・可視化系: `prot_acc`（`ベン図.R` はデフォルトで `SYMBOL` 列）。
- Seurat RDS を使うスクリプト（snAtlas 再解析）は、`RDS_PATH` や `CELLTYPE_COL`（セルタイプ列）、`STAGE_COL`（任意）を環境に合わせて指定する．`input_data/` や `results_*` は任意の出力ディレクトリに変更可能．
- 主な出力:
...
```
必要に応じて各スクリプト内の閾値（例: `PV_CUTOFF`, `TOP_PCT`, 背景モード `BG_MODE`）を調整する必要がある．
