...
# ============================================================
# Pseudo-Volcano Plot Generator (DMSO vs. Mock)
#
# Description:
#   Generates a scatter plot comparing Log2 Fold Change vs. Scaled Abundance.
#   - X-axis: Log2 Fold Change (DMSO vs. Mock).
#   - Y-axis: Scaled Abundance (Conditional selection).
#       * If Log2FC > 0: Uses abundance from DMSO sample.
...
COL_LOG2FC <- "Log2FC_DMSO"
COL_POS    <- "Abundances (Scaled): F7: Sample, BIL7-DMSO"    # Sample for positive FC
...
OUT_PNG    <- sub("\\.xlsx$", "_pseudo_volcano_DMSO_mock.png", IN_XLSX)
OUT_HITS   <- sub("\\.xlsx$", "_hits_DMSO_vs_mock.xlsx", IN_XLSX)
...
p <- ggplot(plot_df, aes(x = Log2FC, y = Abund_sel, color = PlotClass)) +
  ...
  # Labels
  labs(
    title = "Pseudo volcano (Log2FC_DMSO vs scaled abundance)",
    x = "Log2FC_DMSO",
    y = "Scaled Abundance ( + : DMSO / - : Mock )",
    color = "Category"
  ) +
...
# 4. Export Hit Lists
# ==============================================================================
# Extract hits for DMSO (Positive FC)
hits_dmso <- plot_df %>%
  filter(Hit, Series == "DMSO") %>%
